...
# Banc de RENDU fenêtré (pas headless : le GPU compte) dans le vrai monde (carte 3D et centre-ville), joueur et caméra
# réels. Situation signalée par le joueur : caméra au coin du centre-ville tournée vers son cœur, donc toute la ville
# dans le champ (far 4000 m, FOV horizontal ~107° en 16:9).
...
#  --verify-occlusion  : vues fixes (coins au sol et en hauteur ; dans le centre-ville : rues, toits, devant les façades
#                        et les vitrines des boutiques) et trajets en mouvement dans le centre-ville rendus sans
...
const DOWNTOWN := Rect2(-900.0, -470.0, 880.0, 595.0)   # emprise du centre-ville (DowntownSpec), vues de vérification
...
func _verify_paths() -> Array:
	...
	for e in circuit.edges.size():
		var a: Vector3 = circuit.nodes[circuit.edges[e]["a"]]
		var b: Vector3 = circuit.nodes[circuit.edges[e]["b"]]
		if circuit.edges[e]["length"] >= 20.0 and DOWNTOWN.has_point(Vector2(a.x, a.z)) and DOWNTOWN.has_point(Vector2(b.x, b.z)):
			long_edges.append(e)
	_free_cam.make_current()
	var paths := []
	for p in 8:
...
func _verify_views() -> Array:
	...
	# rues et toits : nœuds du Circuit dans le centre-ville (seul quartier à boîtes d'occultation)
	var circuit: Array = []
	for n: Vector3 in _world.get_node("Circuit").nodes:
		if DOWNTOWN.has_point(Vector2(n.x, n.z)):
			circuit.append(n)
	for i in 14:
	...
	# Devant la façade d'un bâtiment du centre-ville (côté +Z local, celui de la rue ; rez-de-chaussée à 5 m), en face
	# puis en biais ; et devant les vitrines des boutiques visitables.
	var buildings := _world.get_node("Downtown/Buildings").get_children()
	for i in 8:
		var mi := buildings[rng.randi() % buildings.size()].get_node("Mesh") as MeshInstance3D
		var box := mi.mesh.get_aabb()
		var c := box.get_center()
		var side := rng.randf_range(-6.0, 6.0)
		views.append(["facade", mi.global_transform * Vector3(c.x + side, 1.7, box.end.z + 5.0), mi.global_transform * Vector3(c.x - side, 1.5, box.end.z - 8.0)])
	for shop: Node3D in _world.get_node("Downtown/Shops").get_children():
		var front := shop.global_transform * Vector3(0.0, 1.7, 14.0)
		views.append(["vitrine", front, shop.global_position + Vector3(0.0, 1.5, 0.0)])
	return views
