...
# lampadaires posés par les lieux (îlot du parking de l'aérogare, 2026-09-21)
const PLACES_HEADS := "res://scenes/world/map/generated/places/lamp_heads.tres"
...
func _halos(cycle: Node) -> void:
	...
	# Même exigence que pour les fenêtres : un halo ombré s'assombrit avec la nuit, au moment précis
	# où il doit s'allumer. Le matériau a porté PER_VERTEX en croyant dire « non éclairé ».
	if not non_eclaire:
...
func _luminaires(world: Node) -> void:
	...
	for chemin in [MAP_HEADS, DOWNTOWN_HEADS, PLACES_HEADS]:
		...
		if res.size() == 0:
			...
			continue
		var carte: bool = chemin == MAP_HEADS or chemin == PLACES_HEADS   # mâts à crosse, comme ceux de la carte
		var hauts := 0
		var bas := 0
		var sans_sol := 0
		var pire_haut := 0.0
