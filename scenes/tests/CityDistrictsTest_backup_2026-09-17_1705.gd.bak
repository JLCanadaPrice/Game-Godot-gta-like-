...
func _check_unique_ids() -> void:
	...
	var total := 0
	var dups := 0
	# Un enfant d'instance éditable (en-tête avec index="…", ex. Road11_Y_Splitter_45 dans chaque district) garde
	# l'identifiant de son nœud dans la scène instanciée : l'éditeur le réécrit à l'identique à chaque sauvegarde.
	...
	print("CITY_SCENE_IDS World.tscn : %d lignes, %d nœuds propres identifiés, %d unique_id en double" % [text.count("\n"), total, dups])
	if dups > 0:
