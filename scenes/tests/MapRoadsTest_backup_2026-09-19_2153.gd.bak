...
func _check_world() -> void:
	...
	var lit_expected := 0
	for i in (traffic.graph as MapTrafficGraph).lit:
		lit_expected += 1
	print("MAP_ROADS_WORLD feux visibles posés par la carte : %d (%d carrefours à feux)" % [traffic.added_lights, lit_expected])
	if lit_expected > 0 and traffic.added_lights < lit_expected * 2:
		_errors.append("feux visibles manquants aux carrefours de la carte (%d)" % traffic.added_lights)
