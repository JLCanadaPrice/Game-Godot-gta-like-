...
const SPAWN_HIGH := 20.0      # m au-dessus du sol : départ en altitude, marche testée au sol à sa verticale
...
func _spawn_walk() -> void:
	...
	var ground := _ground_at(Vector2(spawn.x, spawn.z))
	if ground != INF and spawn.y > ground + SPAWN_HIGH:
		# départ réglé en altitude dans World.tscn (le joueur tombe au lancement) : la marche est testée au sol, à la
		# verticale du départ, et le réglage est signalé
		print("MAP_GATE_SPAWN_NOTE joueur placé à %.0f m au-dessus du sol dans World.tscn : marche testée au sol à la verticale du départ"
				% (spawn.y - ground))
		_player.velocity = Vector3.ZERO
		_player.global_position = Vector3(spawn.x, ground + 1.2, spawn.z)
	await _frames(30)
