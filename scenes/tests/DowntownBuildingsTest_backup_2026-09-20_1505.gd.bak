...
#  - optimisations : boîtes d'occultation posées dans le volume des bâtiments, au ras du sol pour 70 % au moins d'entre
#    eux, bâtiments pris en charge par CityRenderOptimizer (LOD lointain) et
...
func _ready() -> void:
	...
	for building in holder.get_children():
		...
		# boîtes d'occultation (repère du bâtiment, comme son maillage) : dans son volume, au moins une au ras du sol
		var occ_nodes := building.find_children("Occluder*", "OccluderInstance3D", false, false)
		var low := false
		for occ: OccluderInstance3D in occ_nodes:
			var shape := occ.occluder as BoxOccluder3D
			var local := AABB(occ.position - shape.size * 0.5, shape.size)
			low = low or local.position.y <= 1.0
			if not mi.get_aabb().grow(0.01).encloses(local):
				occluders_outside += 1
		occluders += 0 if occ_nodes.is_empty() else 1
		occluders_low += 1 if low else 0
