...
func _place(entry: Dictionary, pos: Vector3, lift := 0.0) -> void:
	...
	# un civil à poids nul ne sort plus en circulation : marqué ici, sinon rien ne le distingue au banc
	var retired: bool = String(entry["role"]) == "civil" and float(entry["poids"]) <= 0.0
	var title := Label3D.new()
	title.text = String(entry["nom"])
	title.modulate = Color(1.0, 0.55, 0.45) if retired else Color(1, 1, 1)
	title.font_size = 64
	...
	var sub := Label3D.new()
	sub.text = "%s · %d variante(s) · poids %.2f%s" % [entry["id"], entry["variantes"], entry["poids"],
			" · RETIRÉ DE LA CIRCULATION" if retired else ""]
	sub.font_size = 48
	...
	sub.modulate = Color(1.0, 0.6, 0.5) if retired else Color(0.85, 0.88, 0.95)
