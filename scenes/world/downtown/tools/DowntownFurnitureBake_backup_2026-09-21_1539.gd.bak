...
# Rien sur les chemins du réseau piéton (mobilier près de la bordure ou contre les façades). Objets fusionnés par bloc de
# CHUNK m et par famille (objets du pack urbain sur une palette partagée, objets à ombre portée, affiches, arbres) en
# maillages statiques : quelques appels de dessin par bloc ; portée de visibilité par famille ; collisions (cylindres,
# boîtes) pour les mâts, troncs, fontaines, abribus et panneaux.
...
const KINDS := {
	...
	"tree_slim": ["res://assets/modular_roads/Tree4.glb", 420.0, true],   # même modèle affiné (TREE_SCALE) : un seul matériau
	...
	"firetruck": [VEH + "Emergency/Veh_Firetruck.glb", 380.0, true],
}

# --- camions des casernes -------------------------------------------------------------------------------------------
# Les quatre `el:Business_FireStation` du centre-ville sont bâties À FLEUR DU TROTTOIR : le mur de façade touche le
# bord de l'intérieur d'îlot, il n'y a aucun parvis entre la caserne et la rue. L'emprise libre relevée au rayon
# (21,5 x 5,0 m, sol à 0,20 m) est la COUR, de l'autre côté du bâtiment. C'est donc là que les camions se garent,
# alignés le long de la façade arrière et le nez tourné vers la sortie de cour.
const VEH := "res://assets/vehicle_models_extra/city_vehicles_UNVERIFIED_LICENSE/"
const FIRE_MODEL := "el:Business_FireStation"
const FIRE_SCALE := 1.65       # échelle du catalogue (resources/vehicle_models/city_firetruck.tres), pas une supposition
const FIRE_GAP := 1.0          # m entre l'arrière d'un camion et le nez du suivant
const FIRE_WALL := 1.2         # m entre le mur de la caserne et le flanc du camion
...
var _meshes := {}                    # modèle -> Mesh fusionné, gardé entre les passes
...
func _initialize() -> void:
	...
	print("DOWNTOWN_FURNITURE " + JSON.stringify({"instances": _total(), "par_modele": _counts, "groupes": _instances.size(),
...
# Deux camions nez à queue par caserne, dans la cour, alignés sur la façade arrière et le nez vers la sortie.
# Tout est pris sur des mesures et non sur des cotes écrites à la main : le gabarit du camion vient de l'AABB du
# maillage fusionné, la cour vient des rects de `buildings.json` et du plan, l'assiette vient de WALK_Y.
func _fire_stations() -> void:
	var mesh := _mesh("firetruck")
	var box := mesh.get_aabb()
	var size := box.size * FIRE_SCALE                      # 3,06 x 3,33 x 8,92 m mesurés
	var longueur := size.z * 2.0 + FIRE_GAP
	for i in buildings.size():
		var b: Dictionary = buildings[i]
		if String(b["model"]) != FIRE_MODEL:
			continue
		var rect := Rect2(float(b["rect"][0]), float(b["rect"][1]), float(b["rect"][2]), float(b["rect"][3]))
		var inner: Rect2 = (layout.blocks[int(b["block"])] as Dictionary)["interior"]
		# Côté rue : celui des deux bords en Z où la caserne est à fleur de l'intérieur d'îlot (les casernes sont
		# posées en frontage, donc collées au trottoir). La cour est en face.
		var vers := 1.0 if absf(rect.position.y - inner.position.y) < absf(rect.end.y - inner.end.y) else -1.0
		var dos: float = rect.end.y if vers > 0.0 else rect.position.y
		# profondeur de cour : jusqu'au bâtiment d'en face du même îlot s'il y en a un, sinon jusqu'au bord de l'îlot
		var limite: float = inner.end.y if vers > 0.0 else inner.position.y
		for j in buildings.size():
			if j == i:
				continue
			var o: Dictionary = buildings[j]
			if int(o["block"]) != int(b["block"]):
				continue
			var r2 := Rect2(float(o["rect"][0]), float(o["rect"][1]), float(o["rect"][2]), float(o["rect"][3]))
			if r2.end.x <= rect.position.x or r2.position.x >= rect.end.x:
				continue                                   # pas en vis-à-vis de la caserne
			if vers > 0.0 and r2.position.y >= dos:
				limite = minf(limite, r2.position.y)
			elif vers < 0.0 and r2.end.y <= dos:
				limite = maxf(limite, r2.end.y)
		var profondeur := absf(limite - dos)
		if profondeur < FIRE_WALL + size.x + 0.4 or rect.size.x < longueur:
			print("DOWNTOWN_FURNITURE caserne du bloc %d ecartee : cour de %.2f m pour %.2f m necessaires, facade %.2f m pour %.2f m"
					% [int(b["block"]), profondeur, FIRE_WALL + size.x + 0.4, rect.size.x, longueur])
			continue
		# nez vers la sortie de cour : le bord en X de l'îlot le plus proche de la caserne
		var vers_x := -1.0 if (rect.position.x - inner.position.x) <= (inner.end.x - rect.end.x) else 1.0
		# Le modèle a son nez sur +Z (le catalogue lui applique un lacet de 180° pour l'aligner sur l'avant d'une
		# voiture, qui est -Z) : viser `n` demande donc atan2(n.x, n.z).
		var rot := Basis(Vector3.UP, atan2(vers_x, 0.0))
		var z := dos + vers * (FIRE_WALL + size.x * 0.5)
...
const TREE_SCALE := {"tree_slim": Vector3(0.72, 0.8, 0.72), "tree_tall": Vector3(1.0, 1.0, 1.0)}


func _add(kind: String, t: Transform3D) -> void:
	if TREE_SCALE.has(kind):
		# variantes d'un même arbre : échelle intégrée au maillage fusionné, légère variation de hauteur
		var s: Vector3 = TREE_SCALE[kind] * rng.randf_range(0.9, 1.1)
		t = Transform3D(Basis.from_scale(s) * t.basis, t.origin)
	var down := MapSpec.DOWNTOWN
...
func _mesh(kind: String) -> Mesh:
	if _meshes.has(kind):
		return _meshes[kind]                 # _fire_stations a besoin du gabarit avant l'écriture de la scène
...
# Familles de fusion : même matériau (palette partagée du pack urbain, ou matériaux des arbres), même portée, même ombre.
# Lampadaires sans ombre portée (mâts fins : ombre à peine visible, mais chaque maillage qui projette une ombre est
# redessiné dans chaque cascade de l'ombre du soleil) ; fontaines, abribus et arbres gardent la leur.
const FAMILIES := {
	"objets": {"range": 380.0, "shadow": false, "palette": true},
	"objets_ombres": {"range": 320.0, "shadow": true, "palette": true},
	"affiches": {"range": 900.0, "shadow": false, "palette": true},
	"arbres": {"range": 420.0, "shadow": true, "palette": false},
	# Camions de caserne : matériau du pack de véhicules, pas la palette urbaine. Portée au barème des lieux
	# (hauteur x 115, bornée 250..1500) sur les 3,327 m mesurés du camion à l'échelle 1,65, soit 383 m -> 380.
	"camions": {"range": 380.0, "shadow": true, "palette": false},
}
const FAMILY_OF := {"fountain": "objets_ombres", "bus_stop": "objets_ombres", "billboard_roof": "affiches", "billboard_pole": "affiches",
...
func _write_scene() -> String:
	var root := Node3D.new()
	root.name = "Furniture"
	# nombre d'objets posés, relu par les tests : les maillages fusionnés ne le gardent pas
	root.set_meta(&"instances", _total())
	root.set_meta(&"par_modele", _counts.duplicate())
	var palette: Material = load(PALETTE_MATERIAL)
	var merged := {}   # "famille|i|j" -> {matériau: SurfaceTool}
	var keys := _instances.keys()
	keys.sort()
	for key: String in keys:
		var parts := key.split("|")
		var kind := parts[0]
		var family: String = FAMILY_OF.get(kind, "objets")
		var mkey := "%s|%s|%s" % [family, parts[1], parts[2]]
		if not merged.has(mkey):
			merged[mkey] = {}
		var mesh: Mesh = _mesh(kind)
		for t: Transform3D in _instances[key]:
			for s in mesh.get_surface_count():
				var mat: Material = palette if FAMILIES[family]["palette"] else mesh.surface_get_material(s)
				if not merged[mkey].has(mat):
					var st := SurfaceTool.new()
					st.begin(Mesh.PRIMITIVE_TRIANGLES)
					merged[mkey][mat] = st
				(merged[mkey][mat] as SurfaceTool).append_from(mesh, s, t)
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(MODELS.path_join("chunks")))
	var mkeys := merged.keys()
	mkeys.sort()
	for mkey: String in mkeys:
		var parts := mkey.split("|")
		var out := ArrayMesh.new()
		for mat in merged[mkey]:
			var st: SurfaceTool = merged[mkey][mat]
			st.set_material(mat)
			st.commit(out)
		var path := MODELS.path_join("chunks/%s_%s_%s.res" % [parts[0], parts[1], parts[2]])
		ResourceSaver.save(out, path)
		var mi := MeshInstance3D.new()
		mi.name = "%s_%s_%s" % [parts[0], parts[1], parts[2]]
		mi.mesh = load(path)
		mi.visibility_range_end = float(FAMILIES[parts[0]]["range"])
