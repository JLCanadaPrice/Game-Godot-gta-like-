...
const CROSSING_SCRIPT := preload("res://scenes/world/RailCrossing.gd")
...
@export var warning_seconds := 18.0      # anticipation de fermeture d'un passage à niveau
@export var warning_min := 140.0         # m : fermeture au plus tard à cette distance, même train lent
@export var crossing_clear := 14.0       # m derrière la queue avant réouverture

var data: MapRailPath
var length := 0.0
var trains: Array = []
var level_crossings: Array = []
...
# Un passage à niveau par entrée du tracé cuit. Un seul matériau pour tous les mâts et toutes les lisses.
func _build_crossings() -> void:
	var red := StandardMaterial3D.new()
	red.resource_name = "barriere_pn"
	red.albedo_color = Color(0.72, 0.13, 0.11)
	red.roughness = 0.7
	for crossing: Dictionary in data.crossings:
		if not crossing.has("road_dir"):
			push_warning("rail_path.tres sans road_dir : relancer RoadBake pour les passages à niveau")
			return
		var c: Node3D = CROSSING_SCRIPT.new()
		add_child(c)
		c.setup(crossing, red)
		level_crossings.append(c)
...
# Un passage se ferme quand une rame l'occupe, vient de le quitter de moins de crossing_clear m, ou l'approche à
# moins de warning_seconds de marche (jamais moins de warning_min m, pour qu'un fret lent ferme assez tôt).
func _update_crossings() -> void:
	for c: Node3D in level_crossings:
		var cs: float = float(c.s)
		var close := false
		for t: Node3D in trains:
			var d := float(t.direction)
			var ahead: float = (cs - float(t.head)) * d
			var behind: float = (cs - float(t.tail())) * d
			if ahead <= 0.0:
				close = close or behind > -crossing_clear
			else:
				close = close or ahead < maxf(warning_min, float(t.speed) * warning_seconds)
		c.set_closed(close)
