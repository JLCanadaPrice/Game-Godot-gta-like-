...
# bâtiments des quartiers (étape 4, generated/buildings/lots.json), routes du modèle RoadNetwork (autoroutes, bretelles,
# artères, plateaux). Réglages d'import écrits avec : lancer avant l'import.
#
...
func _initialize() -> void:
	...
	# bâtiments des quartiers (DistrictsBake) : emprises orientées
	if FileAccess.file_exists(LOTS):
		for lot: Dictionary in JSON.parse_string(FileAccess.get_file_as_string(LOTS)):
			var front := Vector2(sin(float(lot["yaw"])), cos(float(lot["yaw"])))
			var right := Vector2(-front.y, front.x)
			var center := Vector2(float(lot["x"]), float(lot["z"]))
			var hx: float = float(lot["size"][0]) * 0.5
