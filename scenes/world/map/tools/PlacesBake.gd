...
# Têtes des lampadaires posés par les lieux (parking de l'aérogare) : écrites dans places/lamp_heads.tres,
# que StreetLights lit comme ceux de la carte et du centre-ville pour y poser ses vraies lumières.
...
const AIRPORT_MATERIALS := {
	...
	# 2026-09-21 : TOUT le vitrage de l'aérogare et de la vigie, allumé uniformément la nuit, et les
	# plafonniers du porche d'entrée
	"vitrage": "res://scenes/world/airport_vitrage_material.tres",
	"porche": "res://scenes/world/airport_porche_material.tres",
}
...
func _initialize() -> void:
	...
	f.close()
	var tetes := LampHeadsRes.new()
	tetes.heads = lamp_heads
	tetes.aims = lamp_aims
	ResourceSaver.save(tetes, OUT.path_join("lamp_heads.tres"))
	_ensure_in_map()
...
func _airport() -> void:
	...
	# poteaux posés sur le parvis (2 cm sous le dessus de la raquette), pas sur un vide de 2 cm
	for x: float in [442.0, 470.0, 498.0]:
		_box("steel", Vector3(x, top - 0.02, -1207), Vector3(0.35, 5.02, 0.35), STEEL, true)
	_porche(top)
	_parvis_aerogare(top)
	...
	for place: Dictionary in _parking_aerogare(top):
		_garer_sur_place(place, top)
	_balisage_aeroport(top)
	_vitrage_uniforme = false
	_end("Prairie Wind International Airport", "airport")


# PORCHE D'ENTRÉE DE L'AÉROGARE (2026-09-21) : il était noir la nuit. L'auvent est un bloc qu'on a posé
# nous-mêmes, sans luminaire : on lui en pose, encastrés sous sa sous-face (quatre plafonniers de
# 8 x 1,6 m, même calque que le balisage, allumés avec lui), et trois vraies lumières dirigées vers le
# sol, sans ombre, dans le même groupe : allumées la nuit seulement. Trois lumières fixes au même endroit,
# c'est un coût borné.
const PORCHE_PLAFONNIERS := [447.5, 462.5, 477.5, 492.5]
const PORCHE_LUMIERES := [452.0, 470.0, 488.0]


func _porche(top: float) -> void:
	var y := top + 5.0 - 0.02
	for x: float in PORCHE_PLAFONNIERS:
		_air_quad("porche", Vector3(x - 4.0, y, -1210.8), Vector3(x + 4.0, y, -1210.8), Vector3(x + 4.0, y, -1209.2),
				Vector3(x - 4.0, y, -1209.2), Vector3.DOWN)
	for x: float in PORCHE_LUMIERES:
		var l := SpotLight3D.new()
		l.name = "Porche_%d" % int(x)
		l.position = Vector3(x, top + 4.85, -1210.0)
		l.rotation = Vector3(-PI * 0.5, 0.0, 0.0)          # regarde vers le bas
		l.spot_angle = 62.0
		l.spot_range = 9.0
		...
		l.visible = false
		l.add_to_group(&"airport_glow", true)
		place.add_child(l)
	stats["porche_lumieres"] = PORCHE_LUMIERES.size()


# PARVIS DE L'AÉROGARE (2026-09-21). Entre la façade côté ville, la route d'accès, sa raquette et le
# parking, il restait 11 à 17 m de pelouse : on sortait de l'aérogare dans l'herbe, et le porche d'entrée
# était planté dedans. Relevé au rayon avant d'écrire (sonde jetable, pas de 2 m sur x 250..660,
# z -1216..-1120) : terrain 5,00 m, route d'accès et raquette 5,30 m, bordure de la raquette 5,45 m,
# parking 5,28 m. Le parvis est UNE dalle de béton, au niveau du parking, donc 2 cm SOUS la chaussée :
# elle file sous le bord de la route et sous la raquette, qui la recouvrent sans combat en z, et ce
# sont LEURS bords qui dessinent la limite. Elle mord de 0,5 m sous la façade et sous la tour de
# contrôle, et bute sur le bord nord du parking, au même niveau que lui.
const PARVIS_X := Vector2(370.0, 613.5)       # pignon ouest de l'aérogare -> sous la tour de contrôle
const PARVIS_Z := Vector2(-1214.5, -1197.0)   # sous la façade -> bord nord du parking (sous la route)


func _parvis_aerogare(top: float) -> void:
	var centre := Vector2((PARVIS_X.x + PARVIS_X.y) * 0.5, (PARVIS_Z.x + PARVIS_Z.y) * 0.5)
	var taille := Vector2(PARVIS_X.y - PARVIS_X.x, PARVIS_Z.y - PARVIS_Z.x)
	_paved(centre, taille, top - 0.02, "concrete")
	stats["parvis_aerogare_m2"] = snappedf(taille.x * taille.y, 0.1)
...
func _balisage_aeroport(top: float) -> void:
	...
	# --- aérogare et vigie : leur VITRAGE entier s'allume (cf. _vitrage_allume). Le bandeau de 38 feux
	# blancs posés tous les 10 m à mi-hauteur des façades, et les 4 feux de la vigie, faisaient des points
	# blancs isolés sur une façade noire ; retirés le 2026-09-21.
	# --- tour de contrôle : feu d'obstacle rouge en haut du mât
...
func _parking(center: Vector2, size: Vector2, top: float, two_rows := true, lip := LIP, marge_route := -1.0) -> Array:
	...
	for row: float in rows:
		...
		places.append_array(_rangee(line_center, along, across * row, length, top, marge_route))
	return places


# Une rangée de places perpendiculaires : traits de 5,2 m tous les 2,8 m le long de `along`, et la liste
# des places (centre, direction du nez). Avec `marge_route` >= 0, une place dont le centre est à moins de
# cette marge d'une chaussée ou d'une raquette (même emprise que _near_road) n'est ni dessinée ni
# occupée : c'est ce qui retire les places tombées sur la raquette de l'aérogare (2026-09-21).
func _rangee(line_center: Vector2, along: Vector2, nez: Vector2, length: float, top: float, marge_route := -1.0) -> Array:
	var places: Array = []
	var s := -length * 0.5 + 1.5
	while s <= length * 0.5 - 1.5:
		var marque := line_center + along * s      # (« trait » est un mot réservé de GDScript 4.7)
		if marge_route < 0.0 or not _near_road(marque, marge_route - 1.4):
			_stripe(marque, nez, 5.2, 0.12, top + 0.015, WHITE)
		if s + 2.8 <= length * 0.5 - 1.5:
			var centre := line_center + along * (s + 1.4)
			if marge_route < 0.0 or not _near_road(centre, marge_route):
				places.append({"pos": centre, "nez": nez})
			else:
				stats["places_sur_la_route"] = int(stats.get("places_sur_la_route", 0)) + 1
		s += 2.8
	return places


# Parking de l'aérogare, comme un vrai (2026-09-21) :
#  - les deux rangées de bord du 2026-09-20, SANS les places qui mordaient sur la raquette de la route :
#    trois voitures bleues y étaient garées, sur l'enrobé de la raquette ;
#  - au milieu, une DOUBLE RANGÉE FACE À FACE, nez contre un îlot central, qui ne fait PAS toute la
#    longueur : PARKING_ALLEE_BOUT m d'allée à chaque bout, pour que les voitures fassent le tour ;
#  - des lampadaires SUR L'ÎLOT seulement, là où aucune voiture ne roule.
# Géométrie : parking 126 x 47 m ; rangées de bord de 5,2 m ; double rangée de 2 x 5,2 m autour d'un
# îlot de 2 m ; il reste deux allées de 11,95 m entre les rangées, et 12 m à chaque bout.
const PARKING_CENTRE := Vector2(537, -1173.5)
const PARKING_TAILLE := Vector2(126, 47)
const PARKING_ALLEE_BOUT := 12.0
const PARKING_ILOT := 2.0
const PARKING_ILOT_HAUT := 0.15
const PARKING_LAMPES := 4
const PARKING_MARGE_ROUTE := 3.0
...
func _parking_aerogare(top: float) -> Array:
	var y := top - 0.02
	var places := _parking(PARKING_CENTRE, PARKING_TAILLE, y, true, LIP, PARKING_MARGE_ROUTE)
	var long := PARKING_TAILLE.x - 2.0 * PARKING_ALLEE_BOUT
	for cote: float in [-1.0, 1.0]:
		# rangée nord (cote -1) : nez vers le sud, contre l'îlot ; rangée sud (+1) : nez vers le nord
		var ligne := PARKING_CENTRE + Vector2(0, cote * (PARKING_ILOT * 0.5 + 2.6))
		places.append_array(_rangee(ligne, Vector2(1, 0), Vector2(0, -cote), long, y))
	# îlot : bordure de béton, franchissable par la voiture du joueur comme un trottoir (0,15 m)
	_box("paint", Vector3(PARKING_CENTRE.x, y, PARKING_CENTRE.y), Vector3(long, PARKING_ILOT_HAUT, PARKING_ILOT),
			Color(0.62, 0.62, 0.6), true)
	# lampadaires doubles sur l'îlot, crosses en travers : une tête au-dessus de chaque rangée
	var lampes: Array = []
	for k in PARKING_LAMPES:
		var x := PARKING_CENTRE.x - long * 0.5 + long * (k + 0.5) / PARKING_LAMPES
		var base := Vector3(x, y + PARKING_ILOT_HAUT, PARKING_CENTRE.y)
		var bras := Vector3(0, 0, 1)
		lampes.append(Transform3D(Basis(bras, Vector3.UP, bras.cross(Vector3.UP)), base))
	_lampadaires(LAMPE_DOUBLE, LAMPE_DOUBLE_TETES, lampes, "LampsDouble")
	stats["places_parking_aerogare"] = places.size()
	return places
...
func _lampadaires(chemin: String, tetes: Array, transforms: Array, nom: String) -> void:
	...
	var su: PackedVector2Array = src[Mesh.ARRAY_TEX_UV] if src[Mesh.ARRAY_TEX_UV] != null else PackedVector2Array()
	var si: PackedInt32Array = src[Mesh.ARRAY_INDEX] if src[Mesh.ARRAY_INDEX] != null else PackedInt32Array(range(sv.size()))
	...
	for i in transforms.size():
		...
		pole.name = "%s_%s_%d" % [nom, String(place.get_meta("place_id")), i]
		var forme := BoxShape3D.new()
		forme.size = Vector3(0.3, 6.0, 0.3)
		pole.shape = forme
		pole.position = t.origin + Vector3(0, 3.0, 0)
		body.add_child(pole)
	...
	if not u.is_empty():
		arrays[Mesh.ARRAY_TEX_UV] = u
	arrays[Mesh.ARRAY_INDEX] = idx
	var mesh := ArrayMesh.new()
	mesh.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, arrays)
	mesh.surface_set_material(0, source.surface_get_material(0))
	var chemin_res := OUT.path_join("%s_%s.res" % [String(place.get_meta("place_id")), nom.to_lower()])
	ResourceSaver.save(mesh, chemin_res)
	var mi := MeshInstance3D.new()
	mi.name = nom
	mi.mesh = load(chemin_res)
...
func _quad(part: String, a: Vector3, b: Vector3, c: Vector3, d: Vector3, normal: Vector3, uv: Array = [], color := Color.WHITE, collide := true) -> void:
	if _est_vitrage(color):
		if _vitrage_uniforme:
			_vitrage_allume(a, b, c, d, normal)
		else:
...
# VITRAGE DE L'AÉROPORT, ALLUMÉ D'UN BLOC (2026-09-21). À l'aérogare, le découpage en carreaux tirés
# au hasard donnait des « carreaux bizarres », beiges, posés au hasard sur les deux façades : c'est ce
# que le joueur a demandé de retirer. Ici TOUTES les vitres bleues s'allument la nuit, uniformément, sans
# tirage : le quad de vitrage lui-même, décalé d'un centimètre vers l'extérieur (devant la vitre, derrière
# les meneaux qui sont à 2 cm), dans le calque des lumières d'aéroport, allumé avec le balisage.
var _vitrage_uniforme := false
const VITRAGE_DECALAGE := 0.01


func _vitrage_allume(a: Vector3, b: Vector3, c: Vector3, d: Vector3, normal: Vector3) -> void:
	var n := normal.normalized()
	if n.length_squared() < 0.5:
		return
	var off := n * VITRAGE_DECALAGE
	_air_quad("vitrage", a + off, b + off, c + off, d + off, n)
	stats["vitrages_allumes_m2"] = snappedf(float(stats.get("vitrages_allumes_m2", 0.0)) + ((b - a).cross(d - a)).length(), 0.1)


# Quad dans une « couleur » de lumière d'aéroport (même accumulateur que les feux de balisage).
func _air_quad(couleur: String, a: Vector3, b: Vector3, c: Vector3, d: Vector3, n: Vector3) -> void:
	if not _air_acc.has(couleur):
		_air_acc[couleur] = [PackedVector3Array(), PackedVector3Array(), PackedInt32Array()]
	var acc: Array = _air_acc[couleur]
	var verts: PackedVector3Array = acc[0]
	...
	for q in [a, b, c, d]:
		verts.append(q)
		normals.append(n)
	# même enroulement que _quad : face visible du côté de la normale
...
const PARKING_MODELES := [
	...
	"Microcar/Veh_Microcar_Yellow", "Minivan/Veh_Minivan_White",
]
# (2026-09-21) Ces deux derniers étaient écrits « _01_ » : aucun fichier de ce nom, donc un tirage sur sept
# retombait sur un modèle absent et la place restait vide EN SILENCE (_garer_sur_place sort sans rien dire).
# 36 voitures au lieu d'une cinquantaine sur l'ancien parking ; relevé sur places.json à la section E.
...
const PARKING_MARCHE_ARRIERE := 40  # % de voitures garées en marche arrière
...
func _garer_sur_place(place: Dictionary, top: float) -> void:
	...
	# SENS VARIÉS (2026-09-21) : toutes les voitures étaient garées en marche avant. Une partie est
	# maintenant garée en marche ARRIÈRE (nez vers l'allée), tirage stable comme le reste. Le seuil vise
	# 40 %, mais ce tirage partage des bits avec celui des places vides : mesuré, 49 voitures sur 102.
	var arriere := ((h >> 3) % 100) < PARKING_MARCHE_ARRIERE
	...
	_parked_vehicle(chemin, centre, atan2(-nez.x, -nez.y) + travers + (PI if arriere else 0.0), top + 0.03)
	if arriere:
		stats["garees_en_marche_arriere"] = int(stats.get("garees_en_marche_arriere", 0)) + 1
