...
const ATLAS := {"highway": Vector2(0.0, 0.125), "ramp": Vector2(0.125, 0.206787), "median": Vector2(0.207031, 0.242188),
		...
		# chantier des routes, étape 1 : colonnes de marquage (pixels libres de l'atlas, bornes relevées à la cuisson)
		"junction": Vector2(0.744141, 0.784912), "crosswalk": Vector2(0.786621, 0.915039), "stop": Vector2(0.916504, 0.928223),
		"yield": Vector2(0.929688, 0.953125), "chevron": Vector2(0.954590, 0.983887), "dashed": Vector2(0.985352, 0.997070)}
# largeur couverte par chaque colonne, en m : le marquage se pose à l'échelle 1:1 (u = travers / largeur, v = long / TEX_LENGTH)
const ATLAS_METERS := {"junction": 3.5, "crosswalk": 11.0, "stop": 1.0, "yield": 2.0, "chevron": 2.5, "dashed": 1.0}
...
func _initialize() -> void:
	...
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--cds="):
			_cds_only = arg.trim_prefix("--cds=")
	model = Model.new()
	...
	_carve(modes)
	_index_roads()
	# avant toute pose de lampadaire : _lamp a besoin du gabarit du mât et de l'index des chaussées
	_load_lamp_meshes()
	for rb in modes:
...
# Hauteur libre au-dessus du point p : distance de p à la SOUS-FACE de la chaussée la plus basse qui
# lui passe dessus, INF si le ciel est dégagé. Pendant de _road_under, même index de 16 m.
#
# On interroge le MODÈLE de réseau, pas un rayon : la cuisson tourne avant que la scène existe, il
# n'y a rien à sonder. Le modèle, lui, connaît déjà tous ses ouvrages — c'est le même index qui sert
# à décider où poser une pile.
#
# `reach` est le débord de l'objet qu'on veut loger : un mât mince ne gêne pas au même endroit qu'une
# crosse d'1,27 m. Le seuil de 0,5 m écarte les chaussées voisines qui passent au ras du point sans
# être un ouvrage : en deçà, ce n'est pas un tablier au-dessus, c'est le sol d'à côté.
func _road_over(p: Vector3, owner, reach: float) -> float:
	var free := INF
	...
	for dz in range(-1, 2):
		for dx in range(-1, 2):
			for hit: Vector2i in road_index.get(key + Vector2i(dx, dz), []):
				...
				var under := q.y - Network.DECK - p.y
				if under < 0.5:
					continue
				if Vector2(q.x - p.x, q.z - p.z).length() < float(other.width) * 0.5 + reach:
					free = minf(free, under)
	return free


# Un lampadaire tient-il ici ? Il faut Network.CLEARANCE au-dessus du pied — les 7,00 m que le
# réseau garantit déjà sous un viaduc — et au minimum la hauteur du mât, mesurée sur son maillage.
# Sans ce contrôle, RoadBake plantait des mâts sous un tablier : deux de terre-plein en tranchée
# avec 4,95 m pour un mât de 6,30 m (luminaire à 5 mm de la dalle) et deux de trottoir sous le
# viaduc de l'est, luminaire pendu à 0,49 et 0,63 m sous la sous-face.
func _lamp_fits(part: String, base: Vector3, owner) -> bool:
	var need: float = maxf(Network.CLEARANCE, float(lamp_top.get(part, 0.0)))
	var free := _road_over(base, owner, float(lamp_reach.get(part, 1.5)))
	if free >= need:
		return true
	stats["lampadaires_refuses"] = int(stats.get("lampadaires_refuses", 0)) + 1
	return false
...
# Plateau de carrefour (chantier des routes, étape 2) : surface d'enrobé à l'échelle 1:1 découpée sur la grille de
# l'atlas, hauteurs prises sur le pourtour (raccord sans marche avec les chaussées), rebord vertical, angles de
# trottoir arrondis entre deux bras qui en ont, et marquage posé 1,2 cm au-dessus de la chaussée (ligne d'arrêt sur
# les voies qui arrivent, passage piéton sur les bras à trottoir, jamais de ligne d'axe dans le carrefour) ; îlot en
# herbe surélevé pour un rond-point.
const PAD_MARK_RISE := 0.012
const PAD_STOP_FROM := 0.8          # m depuis le bord du plateau
const PAD_STOP_DEPTH := 0.5
const PAD_WALK_FROM := 1.6
const PAD_WALK_DEPTH := 3.0
...
# Surface du plateau : le polygone du pourtour est découpé sur la grille de l'atlas (largeur de la colonne "junction"
...
func _pad_uv(q: Vector2, atlas: Vector2, x0: float, w: float) -> Vector2:
	return Vector2(lerpf(atlas.x, atlas.y, clampf((q.x - x0) / w, 0.0, 1.0)), q.y / TEX_LENGTH)


# Hauteur d'un point du plateau : exactement celle du pourtour quand le point est dessus (raccord sans marche avec les
# chaussées), moyenne pondérée par l'inverse du carré de la distance ailleurs.
func _pad_point(pad: Dictionary, q: Vector2) -> Vector3:
	var rim: PackedVector3Array = pad["rim"]
	var n := rim.size()
	var best := INF
	var on_rim := 0.0
	for k in n:
		...
		var s := Geometry2D.get_closest_point_to_segment(q, pa, pb)
		var d := s.distance_to(q)
		if d < best:
			best = d
			var ab := pb - pa
			var t := 0.0 if ab.length_squared() < 0.0001 else clampf((s - pa).dot(ab) / ab.length_squared(), 0.0, 1.0)
			on_rim = lerpf(a.y, b.y, t)
	if best < 0.05:
		return Vector3(q.x, on_rim, q.y)
	var center: Vector3 = pad["center"]
	var d2 := Vector2(center.x - q.x, center.z - q.y).length_squared() + 1.0
	var sum := center.y / d2
	var weight := 1.0 / d2
	for p in rim:
		var d3 := Vector2(p.x - q.x, p.z - q.y).length_squared() + 1.0
		sum += p.y / d3
		weight += 1.0 / d3
...
# plateau à la hauteur du trottoir, avec sa bordure côté chaussée.
func _pad_corners(pad: Dictionary) -> void:
	var station: Dictionary = net.stations[pad["id"]]
	var ends: Array = station["ends"]
	var rim: PackedVector3Array = pad["rim"]
	var n := rim.size()
	var center: Vector3 = pad["center"]
	var atlas: Vector2 = ATLAS["sidewalk"]
	var rise := Vector3(0, Network.SIDEWALK_RISE, 0)
	for gap: Dictionary in pad.get("gaps", []):
		var a: int = gap["arm_a"]
		var b: int = gap["arm_b"]
		if a >= ends.size() or b >= ends.size():
			continue
		if not bool((ends[a] as Dictionary).get("sidewalk", false)) or not bool((ends[b] as Dictionary).get("sidewalk", false)):
			continue
		var count: int = gap["count"]
		var first: int = gap["from"]
		var along := 0.0
		for s in count:
			var p0: Vector3 = rim[(first + s) % n]
			var p1: Vector3 = rim[(first + s + 1) % n]
			var o0 := Vector3(p0.x - center.x, 0.0, p0.z - center.z).normalized() * Network.SIDEWALK_WIDTH
			var o1 := Vector3(p1.x - center.x, 0.0, p1.z - center.z).normalized() * Network.SIDEWALK_WIDTH
			var v0 := along / TEX_LENGTH
			along += p0.distance_to(p1)
			var v1 := along / TEX_LENGTH
			_quad("asphalt", p0 + rise, p1 + rise, p1 + rise + o1, p0 + rise + o0, Vector3.UP,
					[Vector2(atlas.x, v0), Vector2(atlas.x, v1), Vector2(atlas.y, v1), Vector2(atlas.y, v0)])
			_quad("concrete", p0, p1, p1 + rise, p0 + rise, Vector3(center.x - p0.x, 0.0, center.z - p0.z).normalized())
...
const CDS_KERB_HEIGHT := 0.15       # hauteur des trottoirs du réseau (Network.SIDEWALK_RISE) : le trottoir du bulbe
const CDS_KERB_WIDTH := 0.30        # est au même niveau que celui des rues
const CDS_WALK_WIDTH := 1.80        # trottoir continu tout autour du bulbe (photo de référence 4)
...
func _cul_de_sac(pad: Dictionary) -> void:
	...
	for p in pad["rim"] as PackedVector3Array:
		radius = maxf(radius, Vector2(p.x - center.x, p.z - center.z).length())
	# essai d'abord sur un seul bulbe (--cds=<id>) avant de l'appliquer aux 113 autres
	var full := _cds_only == "" or String(pad["id"]) == _cds_only
	var edge: Vector2 = ATLAS["stop"]
	var walk: Vector2 = ATLAS["sidewalk"]
	var walk_u := lerpf(walk.x, walk.y, clampf(CDS_WALK_WIDTH / float(Network.WIDTHS["sidewalk"]), 0.0, 1.0))
	...
	for poly: PackedVector2Array in _pad_polygons(pad):
		for k in poly.size():
			...
			# trottoir dallé derrière la bordure, puis jupe jusqu'au terrain : le bulbe n'est plus du bitume posé
			# contre l'herbe (photo de référence 4)
			var v0 := arc / TEX_LENGTH
			arc += q0.distance_to(q1)
			var v1 := arc / TEX_LENGTH
			if full:
				var wa := a + up + oa * CDS_KERB_WIDTH
				var wb := b + up + ob * CDS_KERB_WIDTH
				var oa2 := wa + oa * CDS_WALK_WIDTH
				var ob2 := wb + ob * CDS_WALK_WIDTH
				_quad("asphalt", wa, wb, ob2, oa2, Vector3.UP,
						[Vector2(walk.x, v0), Vector2(walk.x, v1), Vector2(walk_u, v1), Vector2(walk_u, v0)])
				var da := clampf(oa2.y - _sample(heights, Vector2(oa2.x, oa2.z)) + 0.3, LIP, 12.0)
				var db := clampf(ob2.y - _sample(heights, Vector2(ob2.x, ob2.z)) + 0.3, LIP, 12.0)
				_quad("concrete", oa2, ob2, ob2 - Vector3(0, db, 0), oa2 - Vector3(0, da, 0), (oa + ob).normalized())
	...
	if not full:
		return
	# lampadaire au fond du bulbe, planté sur le trottoir, crosse tournée vers la chaussée (photo de référence 4).
	# Même modèle et même matériau que les lampadaires de trottoir : il rejoint leur multimesh de cellule.
	var ends: Array = net.stations[pad["id"]]["ends"]
	if ends.is_empty():
		return
	var d2: Vector2 = ends[0]["dir"]
	var away := Vector3(d2.x, 0.0, d2.y).normalized()
	_lamp("lamp_single", center + away * (radius + CDS_KERB_WIDTH + CDS_WALK_WIDTH * 0.5) + Vector3(0, CDS_KERB_HEIGHT, 0), -away)
...
func _mark(a: Vector3, b: Vector3, out: Vector3, slope: float, from: float, depth: float, u0: float, u1: float) -> void:
	var rise := Vector3(0, PAD_MARK_RISE, 0)
	var near := out * from + Vector3(0, from * slope, 0) + rise
	var far := out * (from + depth) + Vector3(0, (from + depth) * slope, 0) + rise
	_quad("asphalt", a + near, b + near, b + far, a + far, Vector3.UP,
			[Vector2(u0, 0.0), Vector2(u1, 0.0), Vector2(u1, depth / TEX_LENGTH), Vector2(u0, depth / TEX_LENGTH)], Color.BLACK, false)
		...
		for k in pts.size():
			...
			match kind:
				"sidewalk":
					...
					if in_entrance:
						continue
					_lamp("lamp_single", base, -side * outer_sign, rb)
				"median":
					var near_ring := false
					...
					if mode[k] != 1 and not near_ring:
						continue
					_lamp("lamp_double", pts[k] + Vector3(0, 0.85, 0), side, rb)
				"ring":
					# anneau parcouru dans le sens trigonométrique : l'extérieur est à droite
					_lamp("lamp_single", pts[k] + side * (rb.width * 0.5 + 0.8), -side, rb)
			since = 0.0


func _lamp(part: String, base: Vector3, arm: Vector3, owner = null) -> void:
	# Rien au-dessus du pied : sinon le mât traverse l'ouvrage et le luminaire éclaire sa sous-face.
	# Le contrôle est ici, et pas sur chacun des sites d'appel, pour qu'aucun n'y échappe.
	if not _lamp_fits(part, base, owner):
		return
	var g := _group(base)
	if not g.has(part):
		g[part] = []
...
# RoadNetwork, remonté de RAIL_TOP pour être à la hauteur du champignon du rail (là où porte une roue), et accompagné
# de son abscisse curviligne, de ses deux culs-de-sac et de ses passages à niveau. Rayon minimal et pente maximale
# sont MESURÉS ici, pas repris d'une constante.
func _write_rail_path() -> void:
	var line: PackedVector2Array = net.rail_line
	var n := line.size()
	if n < 2:
		push_error("voie ferrée absente : rail_path.tres non écrit")
		return
	var res := RailPathRes.new()
	res.step = Network.STEP
	res.gauge = Network.RAIL_GAUGE
	var total := 0.0
	for i in n:
		res.points.append(Vector3(line[i].x, net.rail_heights[i] + RAIL_TOP, line[i].y))
		if i > 0:
			total += line[i].distance_to(line[i - 1])
		res.arc.append(total)
	res.length = total
	var radius := INF
	var grade := 0.0
	for i in range(1, n - 1):
		...
		# rayon du cercle circonscrit : aire = |(b-a) x (c-a)| / 2, R = abc / (4 aire)
		var area := absf((b - a).cross(c - a)) * 0.5
		if area > 0.0001:
			radius = minf(radius, a.distance_to(b) * b.distance_to(c) * c.distance_to(a) / (4.0 * area))
		var run := a.distance_to(b)
		if run > 0.01:
			grade = maxf(grade, absf(net.rail_heights[i] - net.rail_heights[i - 1]) / run)
	res.min_radius = 0.0 if radius == INF else radius
	res.max_grade = grade
	for e: Dictionary in net.rail_ends:
		var p: Vector3 = e["pos"]
		var s := 0.0 if Vector2(p.x, p.z).distance_to(line[0]) < Vector2(p.x, p.z).distance_to(line[n - 1]) else total
		res.ends.append({"pos": Vector3(p.x, p.y + RAIL_TOP, p.z), "dir": e["dir"], "s": s})
	for lc: Dictionary in net.level_crossings:
		var p: Vector3 = lc["pos"]
		res.crossings.append({"pos": p, "s": float(lc["s"]), "half_gap": float(lc["half_gap"]),
				"road": String(lc["road"].id), "road_width": float(lc["road"].width),
				"road_dir": lc["road_dir"], "rail_dir": lc["rail_dir"]})
	var path := OUT.path_join("rail_path.tres")
...
func _quad(part: String, a: Vector3, b: Vector3, c: Vector3, d: Vector3, normal: Vector3, uv: Array = [], color := Color.BLACK, collide := true) -> void:
	...
	if part != "steel" and collide:   # rails, panneaux et marquage : sans collision (pas de marche pour le joueur ni les roues)
		var faces: PackedVector3Array = g["faces"]
...
var lamp_meshes := {}
var lamp_top := {}                # partie -> hauteur du sommet du mât, mesurée sur le maillage
var lamp_reach := {}              # partie -> débord horizontal maximal (crosse + luminaire), mesuré
...
# Modèles de lampadaire, et le gabarit qu'ils occupent, MESURÉ sur leurs sommets. Appelé avant
# _place_lamps, qui a besoin de la hauteur du mât pour savoir s'il tient sous un ouvrage, et
# réutilisé tel quel par _write_scene, qui fusionne ces mêmes maillages.
func _load_lamp_meshes() -> void:
	for pair in [["lamp_single", LAMP_SINGLE], ["lamp_double", LAMP_DOUBLE]]:
		...
		lamp_meshes[pair[0]] = mesh
		inst.free()
		# sommet du mât et débord de la crosse, lus sur l'emprise du maillage : si le modèle change,
		# le contrôle de dégagement suit sans qu'on ait à retoucher un chiffre ici
		var box := mesh.get_aabb()
		lamp_top[pair[0]] = box.end.y
		lamp_reach[pair[0]] = maxf(maxf(absf(box.position.x), absf(box.end.x)),
				maxf(absf(box.position.z), absf(box.end.z)))


func _write_scene() -> void:
	var mats := _materials()
	if lamp_meshes.is_empty():
		_load_lamp_meshes()
