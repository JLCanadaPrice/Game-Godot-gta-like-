...
const LAMP_POST := preload("res://assets/modular_roads/lamp_1.glb")   # mât qui porte le feu, comme au centre-ville
const POST_BELOW_LIGHT := 3.785
...
func merge_into(circuit: CircuitPath) -> void:
	...
	var lights := _world_lights(circuit)
	for node: int in city_degree:
		if int(city_degree[node]) >= 3:
			for e in circuit.node_edges(node):
				if e >= first_edge:
					_add_light(circuit, lights, node, e)
	# carrefours à feux de la carte : un feu par approche (arêtes qui arrivent au carrefour)
	for i in graph.lit:
		var node := remap[i]
		for e in circuit.node_edges(node):
			if bool(circuit.edges[e].get("one_way", false)) and int(circuit.edges[e]["a"]) == node:
				continue
			_add_light(circuit, lights, node, e)


# Feu visible pour l'arête `edge` du carrefour `node` : même pose, relative à sa branche, qu'un feu existant du
# carrefour (enfant d'une tuile de route du centre-ville) ou, à défaut, que le premier feu du centre-ville, tournée
# vers la branche ; recul borné pour rester au bord du plateau.
func _add_light(circuit: CircuitPath, lights: Array[TrafficLight], node: int, edge: int) -> void:
	if lights.is_empty():
		return
	var model: TrafficLight = lights[0]
	for light in lights:
		if light.circuit_node == node:
			model = light
			break
	var model_center := circuit.node_pos(model.circuit_node)
	var center := circuit.node_pos(node)
	var model_dir := _arm_direction(circuit, model.circuit_node, model.circuit_edge)
	...
	relative.origin -= model_center
	if model.circuit_node != node:
		var back := relative.origin.dot(model_dir)
		relative.origin += model_dir * (clampf(back, 12.0, 20.0) - back)
	...
	post.name = "MapTrafficLightPost_%d" % edge
	add_child(post)
	post.global_transform = Transform3D(light.global_transform.basis, light.global_transform.origin - Vector3(0, POST_BELOW_LIGHT, 0))
	added_lights += 1
