...
# DIAGNOSTIC uniquement (session de debug derive/rotation) : desactive le
# terme de compensation roulis/tangage -> force laterale (voir plus bas).
@export var DEBUG_DisableInclineCompensation := false
@export var DEBUG_DisableCamberCoupling := false
...
# Cause confirmee du bercement/roulis persistant (jamais amorti) : "velocity"
# est une vitesse verticale estimee par difference de position finie
# (marqueur $velocity/step), donc bruitee d'un tick a l'autre. Cette
# composante verticale ("linearz" dans VitaVehicleSimulation.suspension())
# alternait de signe quasiment chaque frame physique (mesure : +0.66 puis
# -0.65 m/s d'une frame a l'autre, sans amortissement dans le temps), pour
# une compression "j" quasi stable (~0.043-0.056) -> le terme
# d'amortissement (linearz*damping2, ~2.5-2.8) devenait alors comparable au
# terme de ressort (j*elasticity2, ~2.4-3.0) et le faisait parfois dépasser
# vers le negatif. Le clamp final "if suspforce<0: suspforce=0" (deja
# present, physiquement correct en soi -- un ressort ne peut pas "tirer")
# transformait alors ce bruit de mesure en bascule complete 0 <-> pleine
# force a chaque frame (confirme par instrumentation directe : roue fr,
# ~0N a une frame, ~5.5-6.3N a la suivante, en continu). Lisse ici la
# vitesse verticale AVANT de la transmettre au calcul de suspension, pour
# eliminer ce bruit de derivation numerique sans changer le modele
# physique lui-meme (le ressort/amortisseur reste inchange, seule
# l'estimation de vitesse d'entree est filtree).
var _velocity_y_smooth = 0.0
const VELOCITY_Y_SMOOTH_FACTOR = 0.3
var compress = 0.0
var compensate = 0.0
...
func _physics_process(_delta):
	...
	_velocity_y_smooth = lerpf(_velocity_y_smooth, velocity.y, VELOCITY_Y_SMOOTH_FACTOR)
	...
	if is_colliding():
		...
		var suspforce = VitaVehicleSimulation.suspension(self,S_MaxCompression,A_InclineArea,A_ImpactForce,S_RestLength, elasticity,damping,damping_rebound, _velocity_y_smooth,abs(cast_to.y),global_translation,get_collision_point(),car.mass,ground_bump,ground_bump_height)
		compress = suspforce
...
#		var grav_incline2 = $geometry.global_transform.basis.orthonormalized().xform_inv(Vector3(0,1,0)).z
		...
		# DIAGNOSTIC (Essai C1) : neutralise le couplage roulis/tangage -> force
		# laterale. grav_incline lit le tilt GLOBAL de $geometry par rapport a la
		# gravite ; un tres leger roulis residuel apres l'atterrissage produit ici
		# une force laterale constante (proportionnelle a l'angle, pas a la
		# vitesse) qui ne s'amortit jamais -> suspecte d'etre la derive/rotation.
		if not DEBUG_DisableInclineCompensation:
		...
		# DIAGNOSTIC (Essai C2) : neutralise le couplage camber dynamique -> force
		# laterale (passe 1/2, alimente le glissement/spin de roue "wv").
		if not DEBUG_DisableCamberCoupling:
...
#			skvol *= skvol
	...
	if is_colliding():
		...
		directional_force.y = VitaVehicleSimulation.suspension(self,S_MaxCompression,A_InclineArea,A_ImpactForce,S_RestLength, elasticity,damping,damping_rebound, _velocity_y_smooth,abs(cast_to.y),global_translation,get_collision_point(),car.mass,ground_bump,ground_bump_height)
...
#		var grav_incline = $geometry.global_transform.basis.orthonormalized().xform_inv(Vector3(0,1,0)).x
		...
		# DIAGNOSTIC (Essai C2) : neutralise le couplage camber dynamique -> force
		# laterale (passe 2/2, alimente la force reellement appliquee au chassis).
		if not DEBUG_DisableCamberCoupling:
			distx -= atan2(abs(wv),1.0)*((angle*10.0)*w_size)
