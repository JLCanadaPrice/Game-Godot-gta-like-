...
# --- ANTI-TONNEAU (couple de rappel actif) ---
# Constat de debug (harnais de chute + harnais de conduite headless, session
# dediee) : ce modele de suspension (4 raycasts independants + barre
# anti-roulis faible) ne se remet jamais d'un roulis important. A hauteur de
# chute realiste (~3m, rampe/trottoir) ou lors d'un virage a fond suivi d'un
# freinage, la caisse part carrement en TONNEAU (bascule >90°, confirme par
# mesure directe du vecteur "up" du chassis : up_dot passe negatif).
#
# Detection : deux pistes essayees.
# 1) Le declencheur d'origine (une roue durablement a charge quasi nulle,
#    mesure sur le test de chute) fonctionne pour un impact vertical, mais
#    se met a rester eleve en PERMANENCE des qu'on freine ou accelere en
#    ligne droite (transfert de charge avant/arriere tout a fait normal,
#    pas un signe de tonneau) — verifie sur le test de conduite complet
#    (acceleration + virage + freinage) : la severite issue de ce
#    declencheur montait a 0.3-0.98 en pleine ligne droite. Une correction
#    "toujours quasi a fond" a cause d'un faux signal permanent laisse
#    l'asservissement sature (couple deja au maximum) bien avant qu'un vrai
#    tonneau ne survienne, et incapable de repondre plus fort au bon moment.
# 2) La detection retenue est donc purement l'inclinaison mesuree de la
#    caisse (vecteur "up" du chassis vs la verticale monde), independamment
#    de sa cause (impact vertical OU virage/freinage) : reste a 0 tant que
#    la caisse est proche du niveau (jusqu'a AntiRollover_TiltFloor, en
#    cos(angle)), monte vers 1 quand elle s'incline franchement
#    (AntiRollover_TiltFull). Une pente normale ne s'approche pas de ce
#    seuil, donc pas d'intervention parasite en conduite normale.
#
# Correction : un premier essai appliquait un couple ouvert proportionnel au
# seul deficit de charge (offset_roue x DOWN x gain). A gain faible ça ne
# faisait presque rien ; a gain plus fort ça a carrement RETOURNE la voiture
# sur le toit — un couple en boucle ouverte, sans terme de rappel ni
# d'amortissement, peut depasser la position "a plat" et faire basculer de
# l'autre cote (le faux "settle_time" obtenu venait d'un repos stable...
# a l'envers). Le couple applique ici est donc un vrai asservissement PD sur
# l'inclinaison (vecteur "up" vs la verticale monde) + un amortissement pur
# (dissipatif, jamais un rappel vers une direction cible) de la vitesse
# angulaire complete, le tout mis a l'echelle par la severite ci-dessus. Le
# couple maximal autorise croit lui aussi avec la severite (mineur pour une
# inclinaison legere, bien plus fort pour un vrai tonneau en cours) : conduite
# normale = intervention nulle, tonneau reel = reponse decisive.
@export var AntiRollover_Enabled := true
# Durci radicalement (derniere tentative, cf. tonneau complet observe en
# virage+freinage avec les valeurs precedentes) : intervient plus tot
# (TiltFloor plus proche de 1.0), atteint la severite max plus tot
# (TiltFull plus proche de 1.0), et un couple correcteur beaucoup plus fort
# (Kp/Kd et plafonds x5). Priorite explicite donnee a "ne jamais partir en
# tonneau" sur le feeling physiquement pur.
@export var AntiRollover_TiltFloor := 0.995
@export var AntiRollover_TiltFull := 0.92
@export var AntiRollover_Kp := 40000.0
@export var AntiRollover_Kd := 20000.0
@export var AntiRollover_MaxTorqueMin := 15000.0
@export var AntiRollover_MaxTorqueMax := 150000.0
@export var AntiRollover_Debug := false

var _antiroll_wheels_cache = null

func _anti_rollover_correction(_delta: float) -> void:
	if not AntiRollover_Enabled:
		return

	if _antiroll_wheels_cache == null:
		_antiroll_wheels_cache = []
		for i in get_children():
			if "TyreSettings" in i:
				_antiroll_wheels_cache.append(i)

	if _antiroll_wheels_cache.is_empty():
		return

	var total_load := 0.0
	for w in _antiroll_wheels_cache:
		total_load += w.directional_force.y

	if total_load <= 0.001:
		return # toutes les roues en l'air (saut normal) : ne pas intervenir

	var up : Vector3 = global_transform.basis.y
	var up_dot_now : float = up.dot(Vector3.UP)
	var severity = clampf(
		(AntiRollover_TiltFloor - up_dot_now) / maxf(AntiRollover_TiltFloor - AntiRollover_TiltFull, 0.001),
		0.0, 1.0)

	if severity <= 0.0:
		return

	var tilt_error : Vector3 = up.cross(Vector3.UP)
	var corrective_torque : Vector3 = (tilt_error * AntiRollover_Kp - angular_velocity * AntiRollover_Kd) * severity
	# Garde-fou : quel que soit le gain choisi, on ne peut jamais depasser ce
	# couple max — empeche par construction un emballement/depassement vers
	# le tonneau comme celui observe avec la version en boucle ouverte. Le
	# plafond lui-meme monte avec la severite (voir note ci-dessus).
	var max_torque = lerpf(AntiRollover_MaxTorqueMin, AntiRollover_MaxTorqueMax, severity)
	if corrective_torque.length() > max_torque:
		corrective_torque = corrective_torque.normalized() * max_torque
	apply_torque(corrective_torque)

	if AntiRollover_Debug and Engine.get_physics_frames() % 15 == 0:
		print("ANTIROLL severity=%.4f up_dot=%.4f max_torque=%.1f torque=%s" % [
			severity, up_dot_now, max_torque, corrective_torque])

# Deuxieme ligne de defense (derniere tentative, pragmatique plutot que
# physiquement pure) : reduit de force la vitesse horizontale des lors que
# le braquage est significatif ET que la vitesse depasse un seuil -> moins
# de transfert de charge lateral a dissiper, donc moins de risque de
# depasser ce que l'anti-tonneau peut encore rattraper. Freinage plafonne
# par frame (pas un arret instantane) pour rester jouable.
@export var CorneringGovernor_Enabled := true
@export var CorneringSteerThreshold := 0.3   # fraction de braquage a partir de laquelle ca s'active
@export var CorneringSpeedLimit := 6.0       # m/s au-dela duquel ca freine en virage serre
@export var CorneringGovernor_MaxDecel := 15.0   # m/s^2 max applique par ce freinage

func _cornering_speed_governor(delta: float) -> void:
	if not CorneringGovernor_Enabled:
		return
	var turn_severity := clampf(absf(steer2), 0.0, 1.0)
	if turn_severity < CorneringSteerThreshold:
		return
	var horiz_vel := Vector3(linear_velocity.x, 0.0, linear_velocity.z)
	var speed := horiz_vel.length()
	if speed <= CorneringSpeedLimit:
		return
	var excess := speed - CorneringSpeedLimit
	var decel := minf(excess, CorneringGovernor_MaxDecel * delta)
	linear_velocity -= horiz_vel.normalized() * decel
...
#	gforce = global_transform.basis.orthonormalized().xform_inv(gforce)
	...
	_anti_rollover_correction(delta)
	_cornering_speed_governor(delta)
